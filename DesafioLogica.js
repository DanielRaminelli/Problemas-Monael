...
var arrayDeString = lista_cnae.split(',')
var arrayDeNumeros = [];
for(let x in arrayDeString){
    var y = arrayDeString[x].slice(0,10);
    var a = y.trim();
    arrayDeNumeros.push(a);
}
var arrayFinal = arrayDeNumeros.filter(z => z.length == 10);
var arrayResultado = []
for(let a in arrayFinal){
    var iterador;
    iterador = arrayFinal[a].replaceAll('.','');
    iterador = iterador.replaceAll('-','');
    arrayResultado.push(iterador);
}
for(let b in arrayResultado){
